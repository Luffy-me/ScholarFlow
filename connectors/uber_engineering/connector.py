"""uber_engineering connector — live RSS/HTTP with offline fallback."""
...
async def _live(query: str, limit: int) -> list[SourceDocument]:
    ...
    return docs_from_rss_items(items, connector="uber_engineering", tier=TIER, query=query)[:limit]


class UberEngineeringConnector(RealConnector):
    def __init__(self) -> None:
        super().__init__("uber_engineering", tier=TIER, live_collect=_live)


async def collect(query: str, *, limit: int = 5) -> list[SourceDocument]:
    return await UberEngineeringConnector().collect(query, limit=limit)
