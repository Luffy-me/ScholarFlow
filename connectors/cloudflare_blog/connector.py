"""cloudflare_blog connector — live RSS/HTTP with offline fallback."""
...
async def _live(query: str, limit: int) -> list[SourceDocument]:
    ...
    return docs_from_rss_items(items, connector="cloudflare_blog", tier=TIER, query=query)[:limit]


class CloudflareBlogConnector(RealConnector):
    def __init__(self) -> None:
        super().__init__("cloudflare_blog", tier=TIER, live_collect=_live)


async def collect(query: str, *, limit: int = 5) -> list[SourceDocument]:
    return await CloudflareBlogConnector().collect(query, limit=limit)
