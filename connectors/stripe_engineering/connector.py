"""stripe_engineering connector — live RSS/HTTP with offline fallback."""
...
async def _live(query: str, limit: int) -> list[SourceDocument]:
    ...
    return docs_from_rss_items(items, connector="stripe_engineering", tier=TIER, query=query)[:limit]


class StripeEngineeringConnector(RealConnector):
    def __init__(self) -> None:
        super().__init__("stripe_engineering", tier=TIER, live_collect=_live)


async def collect(query: str, *, limit: int = 5) -> list[SourceDocument]:
    return await StripeEngineeringConnector().collect(query, limit=limit)
