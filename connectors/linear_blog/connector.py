"""linear_blog connector — live RSS/HTTP with offline fallback."""
...
FEED_URL = "https://linear.app/rss/changelog"
TIER = 2
...
async def _live(query: str, limit: int) -> list[SourceDocument]:
    ...
    return docs_from_rss_items(items, connector="linear_blog", tier=TIER, query=query)[:limit]


class LinearBlogConnector(RealConnector):
    def __init__(self) -> None:
        super().__init__("linear_blog", tier=TIER, live_collect=_live)


async def collect(query: str, *, limit: int = 5) -> list[SourceDocument]:
    return await LinearBlogConnector().collect(query, limit=limit)
